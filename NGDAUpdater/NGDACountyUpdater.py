        ...
        for line in ReadFile:
            if re.search('gmd:linkage', line, flags=0):
                ...
                if characterSetCounter == 0:
                    ...
                    NewFile.write('<gco:CharacterString>TIGER/Line Shapefile, Current, Series Information for the </gco:CharacterString>\n')
                    NewFile.write('</gmd:parentIdentifier>\n')
                ...
                if FirstTitle == 'Yes':
                    ...
                    #NewFile.write('<!-- Check 2 -->\n')
                else:
                    NewFile.write(line)
            elif re.search('</gmd:title>', line, flags=0):
                endTitleCounter += 1
                #NewFile.write('<!-- if #10 endTitleCounter: ' + str(endTitleCounter) + ' -->\n')
                if endTitleCounter == 4 or endTitleCounter ==1:
                   continue
                else:
                    NewFile.write(line)
            ...
            elif re.search(' <gco:CharacterString>.iso.xml</gco:CharacterString>', line, flags=0):
                RevFile=FileNameCorrector(file,OutFile)
                # NewFile.write('<!-- if #3 -->\n')
                NewFile.write( RevFile)
              ...
              #      endTitleCounter += 1
               # else:
                ...
                if keywordCounter <   PrePlace:
                    #NewFile.write('<!--- should be inserted' + str(keywordCounter) +'-->\n')
                    KeywordGood = 'yes'
                    NewFile.write(line)
                    DescriptiveKeywordsInd = 'off'

                else:
                    #NewFile.write('<!--- should be Notttttttttttttttttttttttttt be inserted ' + str(keywordCounter) +'-->\n')
                    keywordind = 'yes'
                ...
                if descriptiveKeywordsCounter != 1 and descriptiveKeywordsCounter != 3:
                    #NewFile.write('<!-- Pre line in the if statement (descriptiveKeywordsCounter)' + str(descriptiveKeywordsCounter) + ' -->\n')
                    NewFile.write(line)

                #NewFile.write('<!-- Inserted line  (/gmd:descriptiveKeywords) descriptiveKeywordsCounter' + str(descriptiveKeywordsCounter) + '-->\n')
                if descriptiveKeywordsCounter==2:#was2
                    DescriptiveKeywordsInd = 'off'
                    ...
                    #NewFile.write('<!-- descriptiveKeywordsCounter(counterSection) ' + str(descriptiveKeywordsCounter) + '-->')
                ...
                if descriptiveKeywordsCounter > 1:
                    DescriptiveKeywordsInd = 'on'
                else:
                    NewFile.write(line)
                ...
                typeConter+=1
                #NewFile.write( '<!-- typeconter =' + str(typeConter) + ' -->\n')
                if typeConter ==4:
                    continue
                else:
                    NewFile.write(line)
                ...
                if typeConter == 4:
                    #typeConter += 1
                    #NewFile.write('<!-- Updated the type counter --->\n')
                    continue
                else:
                    NewFile.write(line)
            elif re.search('<gmd:thesaurusName>',line,flags=0):
                #NewFile.write('<!-- typeconter thes)(in  =' + str(typeConter) + ' -->\n')
                if CitiationInd == 'On':
                    NewFile.write(line)
                elif typeConter == 4:
                    CitiationInd = 'On'
                ...
                #NewFile.write('<!-- typeconter gmd:CI_Citation)  =' + str(typeConter) + ' CitiationInd ' + CitiationInd + ' -->\n')
                if typeConter == 4:
                    #NewFile.write('<!-- Turned the indicator on -->\n')
                    CitiationInd='On'
                    continue
                else:
                    NewFile.write(line)
            elif re.search('gmd:title',line,flags=0):
                #NewFile.write('<!-- typeconter (gmd:title)(in  =' + str(typeConter) + ' -->\n')
                if typeConter == 4:
                    CitiationInd = 'On'
                    continue
                else:
                ...
                if typeConter == 4:
                    ...
                    NewFile.write(line)
            elif re.search('gmd:LI_Source',line,flags=0):
                typeConter +=1
                NewFile.write(line)
