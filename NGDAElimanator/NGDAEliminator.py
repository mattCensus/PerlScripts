...
#path = "W:/2016TigerFiles/2017files/tbg"
path='C:/Users/mattp/Desktop/WorkFiles/XML Files/ISO Files/2017Files/scsd'
...
for fileA in configfiles:
    ...
    for x in f:
        if re.search('</gmd:descriptiveKeywords>', x, flags=0):
            ...
            nf.write("      <gco:Date>%s</gco:Date>\n" % PresentDate2)
        elif re.search('2017-12-04', x, flags=0):
            nf.write("      <gco:Date>%s</gco:Date>\n" % PresentDate2)
...
#os.chown(fileB,777)
os.chdir('/')
#FileSizeb=str(os.path.getsize(fileB))
#FileSizeC=str(os.path.getsize(FileC))

#print("fileSizeb %s: " % FileSizeb)
#print("FileSizeC %s: " % FileSizeC)

#if os.path.exists(fileB):
#    if os.path.getsize(fileB) >0:
#        print ("checking to see if both files have been completely copied over:::")
#        while os.path.getsize(FileC) != os.path.getsize(fileB):
#            print ("waiting for the process to finish")
#            print ("file size is " + os.path.getsize(fileB) )
#            time.sleep(5)
